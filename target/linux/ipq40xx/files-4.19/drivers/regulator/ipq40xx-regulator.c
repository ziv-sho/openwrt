...
//
// Driver for IPQ40xx SD/MMC controller's I/O voltage regulator
...
static struct regulator_desc vmmc_regulator = {
	.name		= "vmmcq",
	.ops		= &ipq40xx_regulator_voltage_ops,
...
const struct regmap_config ipq40xx_vmmcq_regmap_config = {
	.reg_bits	= 32,
	.reg_stride	= 4,
	.val_bits	= 32,
};
...
static int ipq40xx_regulator_probe(struct platform_device *pdev)
{
	...
	rmap = devm_regmap_init_mmio(dev, base, &ipq40xx_vmmcq_regmap_config);
	if (IS_ERR(rmap))
		return PTR_ERR(rmap);
...
static const struct of_device_id regulator_ipq40xx_of_match[] = {
	{ .compatible = "qcom,ipq40xx-vqmmc-regulator", },
	{},
};
...
module_platform_driver(ipq40xx_regulator_driver);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Mantas Pucka <mantas@8devices.com>");
MODULE_DESCRIPTION("IPQ40xx VQMMC voltage regulator");