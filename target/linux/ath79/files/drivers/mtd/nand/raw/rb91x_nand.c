...
static const struct of_device_id rb91x_nand_dt_match[] = {
	{ .compatible = "mikrotik,rb91x-nand" },
	{ },
};
MODULE_DEVICE_TABLE(of, rb91x_nand_dt_match);

static int rb91x_nand_probe_dt(struct device *dev,
                               struct rb91x_nand_platform_data *pdata)
{
	...
	pdata->gpio_nce = of_get_gpio(np, 0);
	pdata->gpio_ale = of_get_gpio(np, 1);
	pdata->gpio_cle = of_get_gpio(np, 2);
	pdata->gpio_read = of_get_gpio(np, 3);
	pdata->gpio_nrw = of_get_gpio(np, 4);
	pdata->gpio_rdy = of_get_gpio(np, 5);
	pdata->gpio_nle = of_get_gpio(np, 6);
...
static int rb91x_nand_probe(struct platform_device *pdev)
{
	...
	struct rb91x_nand_platform_data *pdata = kzalloc(sizeof(*pdata), GFP_KERNEL);
	struct mtd_info *mtd;
	int ret;
	...
	rb91x_nand_probe_dt(&pdev->dev, pdata);
	
	pr_info("rb91x nand device tree loaded\n");
...
static struct platform_driver rb91x_nand_driver = {
	...
	.driver	= {
		...
		.of_match_table = of_match_ptr(rb91x_nand_dt_match),
		.owner	= THIS_MODULE,
	},
};
